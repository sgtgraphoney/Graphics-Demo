...
import graphics.shadows.ShadowFrameBuffer;
...
public class ShadowMapMasterRenderer {
     ...
     *
     * @param environment - the lists of entities to be rendered. Each list is
     *                 associated with the {@link BasicModel} that all of the
     *                 entities in that list use.
     * @param sun      - the light acting as the sun in the scene.
     */
    public void render(Environment environment, SpotLight sun) {
        shadowBox.update();
        Vector3f sunPosition = sun.getPosition();
        Vector3f lightDirection = new Vector3f(-sunPosition.x, -sunPosition.y, -sunPosition.z);
        prepare(lightDirection, shadowBox);
        entityRenderer.render(environment);
        finish();
    }
